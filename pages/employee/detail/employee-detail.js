...
Page({
  ...
  onShow: function (e) {
    ...
    WXAPI.applicantResumeDetail(this.data.applicantId).then(res => {
      if (res.code === 200) {
        //  console.log(res.data)
        if (res.data.certificateList) {
  ...
  */
  ...
  launchChat: function () {
    console.log(this.data.applicantId)
     wx.navigateTo({
        url: '/pages/chat/index?jobId=' + 1 + '&applicantId=' + this.data.applicantId
      })
    // WXAPI.getJobInfoByDelivery(this.data.deliveryId).then(res => {
    //   wx.navigateTo({
    //     url: '/pages/chat/index?jobId=' + res.data.jobId + '&applicantId=' + this.data.applicantId
    //   })
    // })
